...
function generateLatex() {
    ...
    // Certifications
    let certSec = "";
    if (state.enableCertifications && state.certifications.length > 0) {
        let items = "";
        state.certifications.forEach(item => {
            let linkStr = item.link ? ` | \\href{${item.link}}{Certificate}` : "";
            items += `  \\item \\textbf{${escapeLatex(item.title)}}, ${escapeLatex(item.organization)} \\hfill [${escapeLatex(item.date)}]${linkStr}\n`;
        });
        certSec = `\\resheading{\\textbf{CERTIFICATIONS} }\n\\vspace{-0.4cm}\n\\begin{itemize}\\itemsep\\isep\n${items}\\end{itemize}\n\n`;
    }
    latex = latex.replace(/<<CERTIFICATIONS>>/g, certSec);
    ...
    if (state.projects.length > 0) {
        ...
        state.projects.forEach(pj => {
            ...
            if (pj.descriptions && pj.descriptions.length > 0) {
                projItems += `\\begin{itemize}\\itemsep  \\isep\n`;
                pj.descriptions.forEach(desc => {
                    if (desc.trim()) {
                        projItems += `   \\item ${escapeLatex(desc)}\n`;
                    }
                });
...
function generateVisualPreview() {
    ...
    if (state.enableCertifications && state.certifications.length > 0) {
        ...
        state.certifications.forEach(cert => {
            let linkStr = cert.link ? ` | <a href="${cert.link}">Link</a>` : "";
            html += `<li>
                <div class="item-header">
                    <strong>${cert.title}</strong>
                    <span>[${cert.date}]</span>
                </div>
                <div class="item-subheader">${cert.organization}${linkStr}</div>
            </li>`;
        });
        html += `</ul>`;
    }
...
document.getElementById("enable-certifications").addEventListener("change", (e) => {
    state.enableCertifications = e.target.checked;
    document.getElementById("certifications-content").classList.toggle("active", state.enableCertifications);
    generateLatex();
});
...
// Projects
document.getElementById("add-pj-desc-btn").addEventListener("click", () => {
    const container = document.getElementById("pj-descriptions-container");
    const div = document.createElement("div");
    div.className = "dynamic-input-row";
    div.innerHTML = `
        <input type="text" class="pj-desc-input" placeholder="Description Line ${container.children.length + 1}">
        <button type="button" class="btn btn-danger btn-sm remove-pj-desc-btn" style="padding: 0.25rem 0.5rem;">
    ...
    // Add remove listener
    div.querySelector(".remove-pj-desc-btn").onclick = () => {
        div.remove();
        // Re-number placeholders
        Array.from(container.children).forEach((child, i) => {
            child.querySelector("input").placeholder = `Description Line ${i + 1}`;
        });
    };
...
document.getElementById("add-project-btn").addEventListener("click", () => {
    const descInputs = document.querySelectorAll(".pj-desc-input");
    const descriptions = Array.from(descInputs).map(input => input.value).filter(val => val.trim() !== "");
    ...
    // Reset form
    document.querySelectorAll("#project-form input:not(.pj-desc-input)").forEach(i => i.value = "");
    const container = document.getElementById("pj-descriptions-container");
    container.innerHTML = `
        <div class="dynamic-input-row">
            <input type="text" class="pj-desc-input" placeholder="Description Line 1">
        </div>
        <div class="dynamic-input-row">
            <input type="text" class="pj-desc-input" placeholder="Description Line 2">
        </div>
    `;
    updateProjects();
});
...
function updateProjects() {
    renderList("projects-list", state.projects,
        ...
        (idx) => {
            ...
            // Re-populate descriptions
            const container = document.getElementById("pj-descriptions-container");
            container.innerHTML = "";
            el.descriptions.forEach((desc, i) => {
                const div = document.createElement("div");
                div.className = "dynamic-input-row";
                div.innerHTML = `
                    <input type="text" class="pj-desc-input" placeholder="Description Line ${i + 1}" value="${desc.replace(/"/g, '&quot;')}">
                    ${i >= 0 ? `
                    <button type="button" class="btn btn-danger btn-sm remove-pj-desc-btn" style="padding: 0.25rem 0.5rem;">
                        <i data-lucide="x" style="width: 14px; height: 14px;"></i>
                    </button>` : ''}
                ...
                const removeBtn = div.querySelector(".remove-pj-desc-btn");
                if (removeBtn) {
                    removeBtn.onclick = () => {
                        div.remove();
                        Array.from(container.children).forEach((child, j) => {
                            child.querySelector("input").placeholder = `Description Line ${j + 1}`;
                        });
                    };
                }
            });
            ...
            if (el.descriptions.length === 0) {
                container.innerHTML = `<div class="dynamic-input-row"><input type="text" class="pj-desc-input" placeholder="Description Line 1"></div>`;
            }
            if (window.lucide) lucide.createIcons();
...
document.getElementById("add-certification-btn").addEventListener("click", () => {
    ...
    document.querySelectorAll("#certification-form input").forEach(i => i.value = "");
    updateCertifications();
});

function updateCertifications() {
    renderList("certifications-list", state.certifications,
        (item) => `<div class="list-item-title">${item.title}</div><div class="list-item-subtitle">${item.organization} | ${item.date}</div>`,
        (idx) => {
            const el = state.certifications[idx];
            document.getElementById("cert-title").value = el.title;
            document.getElementById("cert-org").value = el.organization;
            document.getElementById("cert-date").value = el.date;
            document.getElementById("cert-link").value = el.link;
